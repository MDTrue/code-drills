...
if((((typeof dataTypeInput) === "object") && (Array.isArray(dataTypeInput))) === true) {
  dataTypeArray(dataTypeInput)
}

    //if the argument is an obj then call the `dataTypeObj` function with it as an argument
 else if((((typeof dataTypeInput) === "object") && (Array.isArray(dataTypeInput))) === false) {
...
if(typeof( customIsNum(dataTypeInput)) === "number"){
  dataTypeInputInt =customIsNum(dataTypeInput)
  fizzBuzzCalculation(dataTypeInputInt)
}
...
if((typeof dataTypeInput) === "number"){
  fizzBuzzCalculation(dataTypeInput)
    ...
    if((typeof dataTypeInput) === "boolean"){
...
for(var j = 0; j<dataTypeInput.length; j++){
  fizzBuzzDataTypeChecker(dataTypeInput[j])
}
