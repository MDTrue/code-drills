...
function toyArrToObj(arrayOfToys){
  ...
  // ---------- Your Code Here ----------
for(var j = 0; j < arrayOfToys.length; j++){
   toyArrayOfObjs.push({name:arrayOfToys[j]});
}
...
function createCustomObject(objectArr){
 ...
 if(customToyLineObj[objectArr[j].toyLine]){
   customToyLineObj[objectArr[j].toyLine].toyLineToys.push(objectArr[j].title)
   customToyLineObj[objectArr[j].toyLine].totalToysInToyLine +=objectArr[j].stock
 }
 else {
    customToyLineObj[objectArr[j].toyLine] = {
       toyLine: objectArr[j].toyline,
       toyLineToys: [objectArr[j].title],
       totalToysInToyLine: objectArr[j].stock
    }
   }
}
